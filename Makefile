...
KESSOS.EFI: KessOS.elf
	$(OBJCOPY) -j .text -j .sdata -j .data -j .dynamic -j .dynsym -j .rel -j .rela -j .rel.* -j .rela.* -j .reloc --target efi-app-x86_64 $< $@

KessOS.elf: reduced-gnu-efi/gnuefi/crt0-efi-x86_64.o reduced-gnu-efi/gnuefi/libgnuefi.a $(OBJ)
...
run: all ovmf
	...
	cp KESSOS.EFI boot/EFI/BOOT/BOOTX64.EFI
	qemu-system-x86_64 -M q35 -drive file=fat:rw:boot -bios ovmf/OVMF.fd
	rm -rf boot

.PHONY: clean
clean:
	rm -rf KESSOS.EFI KessOS.elf $(OBJ) $(HEADER_DEPS)
